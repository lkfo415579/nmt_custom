...
# print ("29,model_helper,forcing use CPU as embeddings")
# VOCAB_SIZE_THRESHOLD_CPU = 5
...
class TrainModel(
    collections.namedtuple("TrainModel", ("graph", "model", "iterator",
                                          "skip_count_placeholder", "insert_op"))):
    pass
...
class EvalModel(
    collections.namedtuple("EvalModel",
                           ("graph", "model", "src_file_placeholder",
                            "tgt_file_placeholder", "iterator", "insert_op"))):
    pass
...
class InferModel(
    collections.namedtuple("InferModel",
                           ("graph", "model", "src_placeholder",
                            "batch_size_placeholder", "iterator", "insert_op"))):
    pass
...
def load_model(model, ckpt, session, name, insert_op):
    start_time = time.time()
    model.saver.restore(session, ckpt)
    # table initi
    global tables_checker
    if len(insert_op) > 0:
        if not (name in tables_checker):
            tables_checker[name] = True
            for op in insert_op:
                session.run(op)
    # session.run(tf.tables_initializer(name="table_init"))
...
def avg_checkpoints(model_dir, num_last_checkpoints, global_step,
    ...
    with tf.Graph().as_default():
        tf_vars = [
            tf.get_variable(
                v, shape=var_values[v].shape, dtype=var_dtypes[name])
            for v in var_values
        ]
...
def create_or_load_model(model, model_dir, session, name, insert_op):
    """Create translation model and initialize or load parameters in session."""
    latest_ckpt = tf.train.latest_checkpoint(model_dir)
    if latest_ckpt:
        model = load_model(model, latest_ckpt, session, name, insert_op)
    else:
        start_time = time.time()
